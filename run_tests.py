...
def execute_suite_executeable(path: str, timeout_ms: Optional[float]) -> SuiteResult:
    try:
        ...
        return SuiteResult(path=path,
                           result=CrashedSuiteResult(return_val=None,
                                                     msg="Timeout",
                                                     stderr=[]),
                           test_results=[])
...
def parse_suite_footer(returncode: int, stdout: List[str], stderr: List[str]) -> CrashedSuiteResult | OkSuiteResult:
    ...
    if len(stdout) < 3:
        return CrashedSuiteResult(return_val=returncode,
                                  stderr=stderr,
                                  msg="Output too short to contain footer")
    ...
    if match:
        ...
        try:
            ...
            return CrashedSuiteResult(return_val=returncode,
                                      stderr=stderr,
                                      msg="Malformed footer")

        return OkSuiteResult(returncode,
                             reported_test_count,
                             reported_fail_count,
                             reported_ignored_count,
                             stderr)
    else:
        return CrashedSuiteResult(return_val=returncode,
                                  stderr=stderr,
